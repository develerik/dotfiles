...
def main():
  ...
  if err:
    if err.endswith(b'(ERROR) You are not currently signed in. Please run `op signin --help` for instructions\n'):
  ...
  if item is not None:
    if verbose:
      ...
      sys.stderr.write('item: ' + item.decode("utf-8") + '\n')
    parseOpItem(item)
  else:
    (item, err) = opGetItem(host, token)
    if err:
...
def usage(msg=None):
  ...
  To configure git to use this helper, run the following command:
    ...
    git config --global credential.helper '1password -q'
